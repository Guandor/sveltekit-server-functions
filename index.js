...
function generatePaths(functionName, filename) {
  const relativePath = filename
    ? path
        .relative(path.join(process.cwd(), "src"), path.normalize(filename))
        .replace(/\\/g, "/")
    : "unknown";
  ...
  calls
    ...
    .forEach((call) => {
      ...
      transformedContent =
        ...
        fetchCall +
        transformedContent.slice(call.end);
    });
...
function createEndpoint(apiPath, content, serverFunc) {
  const { imports, functionDefinition } = extractFunctionDetails(
    content,
    serverFunc
  );
  ...
  if (
    ...
    fs.readFileSync(apiPath, "utf-8") !== apiContent
  ) {
